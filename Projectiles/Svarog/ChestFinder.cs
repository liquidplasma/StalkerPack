...
{
    internal class ChestFinder : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Draw = false;
            if (Svarog.FindNearestChest())
            {
                if (Svarog.NearestChest != Vector2.Zero && Projectile.Center.Distance(Svarog.NearestChest) >= 120f)
                {
                    Draw = true;
                    Projectile.alpha = 0;
                    Vector2 distance = Player.Center.DirectionTo(Svarog.NearestChest) * 60f;
                    Projectile.Center = Player.MountedCenter + distance;
                    Projectile.position.Y += Player.gfxOffY;
                    Projectile.rotation = Projectile.Center.AngleTo(Svarog.NearestChest) + MathHelper.PiOver2;
                }
            }
        }
    }
